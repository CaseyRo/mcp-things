...
class TestConfiguration:
    ...
    def test_default_settings(self):
        ...
        assert settings.things_fastmcp_host == "127.0.0.1"
        assert settings.things_fastmcp_port == 8009
    ...
    def test_port_validation(self):
        ...
        settings = Settings(things_fastmcp_port=8009)
        assert settings.things_fastmcp_port == 8009

        settings = Settings(things_fastmcp_port=1)
        assert settings.things_fastmcp_port == 1

        settings = Settings(things_fastmcp_port=65535)
        assert settings.things_fastmcp_port == 65535

        # Invalid ports should raise ValidationError
        with pytest.raises(ValidationError):
            Settings(things_fastmcp_port=70000)  # Too high

        with pytest.raises(ValidationError):
            Settings(things_fastmcp_port=0)  # Too low

    def test_host_validation(self):
        """Host should accept valid IP addresses."""
        valid_hosts = ["127.0.0.1", "0.0.0.0", "localhost", "192.168.1.1"]
        for host in valid_hosts:
            settings = Settings(things_fastmcp_host=host)
            assert settings.things_fastmcp_host == host
