...
These tools are aliases to the older API. New integrations should use
the GTD-aligned tools instead.
"""
...
from .formatters import format_todo, format_project
...
from .cache import cached, invalidate_caches_for, CACHE_TTL
...
def register_deprecated_tools(mcp: FastMCP):
    """Register deprecated backward-compatibility tools with the MCP server."""
    ...
    async def get_inbox(ctx: Context = None) -> str:
        ...
        try:
            ...
            log_operation_end(
                "get-inbox", True, time.time() - start_time, count=len(todos)
            )
            return "\n\n---\n\n".join(formatted_todos)
        except Exception as e:
            log_operation_end(
                "get-inbox", False, time.time() - start_time, error=str(e)
            )
            raise
    ...
    async def get_today(ctx: Context = None) -> str:
        ...
        try:
            ...
            formatted_todos = [format_todo(todo) for todo in todos]
            log_operation_end(
                "get-today", True, time.time() - start_time, count=len(todos)
            )
            return "\n\n---\n\n".join(formatted_todos)
        except Exception as e:
            log_operation_end(
                "get-today", False, time.time() - start_time, error=str(e)
            )
            raise

    @mcp.tool(
        name="get-upcoming", annotations=TOOL_ANNOTATIONS["get-upcoming"], timeout=5
    )
    ...
    @mcp.tool(
        name="get-anytime", annotations=TOOL_ANNOTATIONS["get-anytime"], timeout=5
    )
    ...
    @mcp.tool(
        name="get-someday", annotations=TOOL_ANNOTATIONS["get-someday"], timeout=5
    )
    ...
    @mcp.tool(
        name="get-logbook", annotations=TOOL_ANNOTATIONS["get-logbook"], timeout=5
    )
    async def get_logbook(
        period: str = "7d", limit: int = 50, ctx: Context = None
    ) -> str:
    ...
    @mcp.tool(
        ...
        annotations=TOOL_ANNOTATIONS["get-tagged-items"],
        timeout=5,
    )
    ...
    @mcp.tool(
        name="search-todos", annotations=TOOL_ANNOTATIONS["search-todos"], timeout=5
    )
    ...
    @mcp.tool(
        ...
        annotations=TOOL_ANNOTATIONS["search-advanced"],
        timeout=5,
    )
    ...
    async def add_task(
        ...
        try:
            ...
            invalidate_caches_for(
                ["get-inbox", "get-today", "get-upcoming", "get-todos"]
            )
    ...
    @mcp.tool(
        name="add-project", annotations=TOOL_ANNOTATIONS["add-project"], timeout=30
    )
    ...
    @mcp.tool(
        name="update-todo", annotations=TOOL_ANNOTATIONS["update-todo"], timeout=30
    )
    ...
    @mcp.tool(
        name="update-project",
        annotations=TOOL_ANNOTATIONS["update-project"],
        timeout=30,
    )
    ...
    @mcp.tool(
        name="search-items", annotations=TOOL_ANNOTATIONS["search-items"], timeout=10
    )
