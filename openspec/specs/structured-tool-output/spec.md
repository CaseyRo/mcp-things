...
Defines the wire-level contract for every `@mcp.tool` exposed by the Things MCP server: a uniform `ToolEnvelope` JSON payload (`data` / `summary` / `meta`) carried on `structured_content`, alongside a backwards-compatible markdown `TextContent` block. Covers the per-tool typed payloads (`Todo`, `Project`, `Area`, `Tag`, `WriteResult`, `BulkResult`, `FocusResult`, `ReviewReport`, `TriageInsights`, `CacheStats`, `ShowInAppResult`), the `outputSchema` advertised on every `tools/list` entry, and the ChatGPT strict-mode transforms applied to those output schemas in the client-compatibility middleware. Downstream consumers (Arbeitsplatz, ChatGPT, n8n, Claude Desktop, Claude Code) read JSON without prose-parsing while text-only clients keep working byte-for-byte.
